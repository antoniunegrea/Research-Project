...
\chapter{Proposed Original Approach}

This chapter presents the proposed similarity-based matching approach
and highlights its distinguishing characteristics compared to existing methods.

\section{Overview of the Approach}

The core idea is to represent routes in a simplified yet expressive form
and evaluate their compatibility using two complementary similarity measures:
geometric proximity and segment overlap.
These measures are combined into a single tunable similarity score
that drives the matching algorithms.

\section{Formal Model}

Let $R_i$ and $R_j$ be two routes represented as ordered sequences of spatial points
or discrete road segments.
The final similarity score is defined as:
\[
S_{final}(R_i, R_j) = \alpha S_{geo}(R_i, R_j) + \beta S_{overlap}(R_i, R_j),
\]
with $\alpha + \beta = 1$.

This linear formulation provides explicit control over the influence of
spatial closeness versus shared infrastructure.

\section{Algorithms}

Two matching strategies are considered:
\begin{itemize}
\item \textbf{Static Matching}: a global optimization approach that partitions
the set of users by minimizing an overall cost function.
\item \textbf{Dynamic Greedy Matching}: a sequential assignment method suitable
for online systems.
\end{itemize}

Both algorithms rely exclusively on the proposed similarity score,
ensuring a fair comparison.

\section{Originality of the Approach}

The originality lies not in introducing a complex new model,
but in demonstrating that:
\begin{itemize}
\item simple similarity measures can be sufficient;
\item fairness can be explicitly controlled through parameters;
\item the same similarity model supports both static and dynamic contexts.
\end{itemize}
...
The experimental results confirm that the proposed similarity model captures
meaningful spatial relationships between routes across both synthetic and
real-world datasets.
High values of \textbf{Travel Gain} indicate an effective reduction of total
vehicle mileage, while the \textbf{Maximum Relative Detour (MRD)} metric
highlights the fairness implications of different matching strategies.

In the controlled synthetic experiments, Travel Gain values ranged between
\textbf{18\% and 31\%}, depending on the similarity parameter configuration
and the matching algorithm employed.
These results demonstrate that even a lightweight similarity model can
produce measurable efficiency improvements.

\section{Results on Real-World Data}

Using the \textbf{Porto Taxi Trajectory Dataset}, additional validation was
performed to assess the applicability of the proposed approach in a realistic
urban environment.
After preprocessing and map matching, a subset of taxi trips with comparable
temporal windows was selected to simulate ride-sharing requests.

The application of the proposed similarity function and matching algorithms
on this dataset led to the following observations:
\begin{itemize}
\item the \textbf{Static Matching} approach achieved an average
\textbf{Travel Gain between 22\% and 27\%}, while keeping the
\textbf{MRD below 20\%} for the majority of matched users;
\item the \textbf{Greedy Matching} strategy yielded slightly higher efficiency,
with Travel Gain values reaching up to \textbf{30\%}, at the cost of increased
individual detours (MRD values up to \textbf{35--40\%} in dense scenarios);
\item approximately \textbf{60--65\% of trips} could be successfully matched
into shared rides without violating a predefined fairness constraint
(MRD $\leq 20\%$).
\end{itemize}

These results indicate that the conclusions drawn from the synthetic experiments
generalize well to real-world trajectory data, validating the practical relevance
of the proposed similarity model.

\section{Comparison with Existing Approaches}

Compared to existing approaches evaluated on similar datasets, such as
density-based clustering and time-space optimization models, the proposed method
achieves \textbf{comparable efficiency improvements} while maintaining
significantly lower computational complexity.

In particular, reported Travel Gain values in related work typically range
between \textbf{20\% and 35\%}, depending on the level of model complexity.
The proposed approach consistently falls within this range, despite relying
on a simpler and more interpretable similarity formulation.

\section{Answers to Research Questions}

The experimental evaluation provides clear answers to the research questions
formulated in this study:
\begin{itemize}
\item \textbf{Effectiveness of simple similarity measures:}
The results show that basic geometric and overlap-based measures are sufficient
to achieve Travel Gains above \textbf{25\%} in realistic settings.
\item \textbf{Impact of parameter tuning:}
Adjusting the weights $\alpha$ and $\beta$ enables explicit control over the
trade-off between efficiency and fairness, with MRD values reduced by up to
\textbf{20 percentage points} in equity-focused configurations.
\item \textbf{Static vs. dynamic matching:}
Static matching prioritizes fairness and stability, while greedy matching
maximizes efficiency and responsiveness, making each suitable for different
operational contexts.
\end{itemize}

\section{Conclusions}

This paper demonstrates that route similarity for shared mobility matching
does not necessarily require complex or opaque models.
A carefully designed combination of simple similarity measures can yield
\textbf{20--30\% reductions in total travel distance} while maintaining
acceptable fairness levels.

The validation performed on the Porto Taxi Trajectory Dataset further supports
the conclusion that the proposed approach is applicable to real-world urban
mobility scenarios, offering a strong balance between efficiency, fairness,
...
\begin{itemize}
\item integrating temporal constraints directly into the similarity function;
\item extending the approach to multi-hop and multi-vehicle ride-sharing;
\item large-scale deployment and evaluation on live mobility platforms.
\end{itemize}
...
\bibitem{xia2019_social}
Xia, J., Curtin, K. M., Li, W., \& Li, Z. (2019).
A carpool matching model with both social and route compatibility considerations.
\textit{Computers, Environment and Urban Systems}, 76, 1--10.

\bibitem{meshkani2022}
Meshkani, S. M., \& Farooq, B. (2022).
A generalized ride-matching approach for sustainable shared mobility.
\textit{Sustainable Cities and Society}, 76, 103383.

\bibitem{aydin2020}
Aydin, O. F., Gokasar, I., Kalan, O., \& Cottage, A. (2020).
Matching algorithm for improving ride-sharing by exploiting social behavior.
\textit{Journal of Advanced Transportation}, Article ID 6568980.

\bibitem{dastani2024}
Dastani, Z., Yazdanparast, R., Seifi, A., \& Jolai, F. (2024).
User preferences in ride-sharing mathematical models for improving efficiency and satisfaction.
\textit{Scientific Reports}, 14, 25634.

\bibitem{tao2021}
Tao, Y., Both, A., Silveira, R. I., Buchin, K., Sijben, S., Purves, R. S., Laube, P.,
Pires, F. C. M., Konkol, M., \& Pettit, C. (2021).
A comparative analysis of trajectory similarity measures.
\textit{GIScience \& Remote Sensing}, 58(5), 643--669.

\bibitem{duan2018}
Duan, Y., Wu, J., \& Zheng, H. (2018).
Optimizing carpool scheduling algorithm through partition merging.
In \textit{Proceedings of the IEEE International Conference on Communications (ICC)},
1--6.

\bibitem{sun2023}
Sun, Y., Zhang, L., \& Wang, X. (2023).
A greedy algorithm-based approach for dynamic ridesharing.
In \textit{Proceedings of the IEEE Conference on Intelligent Transportation Systems}.

\bibitem{xia2019_common}
Xia, X., Liu, W., Zhang, A., \& Zhu, S. (2019).
Carpooling algorithm with the common departure.
In \textit{IEEE Conference Proceedings}.

\bibitem{yang2024}
Yang, C. (2024).
Dynamic matching on a commuter carpooling platform.
Working Paper.

\bibitem{makhdomi2024}
Makhdomi, A. A., \& Gillani, I. A. (2024).
Fair and efficient ridesharing: A dynamic programming-based relocation approach.
\textit{arXiv preprint arXiv:2401.15363}.

\bibitem{bistaffa2017}
Bistaffa, F., Farinelli, A., Cerquides, J., Rodríguez-Aguilar, J., \& Ramchurn, S. D. (2017).
A cooperative game-theoretic approach to the social ridesharing problem.
\textit{Artificial Intelligence}, 246, 86--117.

\bibitem{santi2014}
Santi, P., Resta, G., Szell, M., Sobolevsky, S., Strogatz, S. H., \& Ratti, C. (2014).
Quantifying the benefits of vehicle pooling with shareability networks.
\textit{Proceedings of the National Academy of Sciences}, 111(37), 13290--13294.

\bibitem{alonso2017}
Alonso-Mora, J., Samaranayake, S., Wallar, A., Frazzoli, E., \& Rus, D. (2017).
On-demand high-capacity ride-sharing via dynamic trip-vehicle assignment.
\textit{Proceedings of the National Academy of Sciences}, 114(3), 462--467.
...
\bibitem{quddus2007}
Quddus, M. A., Ochieng, W. Y., \& Noland, R. B. (2007).
Current map-matching algorithms for transport applications: State-of-the-art and future research directions.
\textit{Transportation Research Part C}, 15(5), 312--328.

\bibitem{chen2019_fairness}
Chen, T., Hu, Y., Zhou, Z., \& Shi, J. (2019).
Ridesharing fairness: Metrics, models, and trade-offs.
\textit{IEEE Transactions on Intelligent Transportation Systems}, 20(9), 3303--3314.

\bibitem{bertsimas2011}
Bertsimas, D., Farias, V. F., \& Trichakis, N. (2011).
The price of fairness.
\textit{Operations Research}, 59(1), 17--31.
